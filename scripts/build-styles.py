#!/usr/bin/env python3
"""Build immutable, root-level CSS bundles without changing the CSS cascade.

Run with --download-fonts once to refresh the locally hosted Google Fonts files.
Subsequent builds run offline. HTML links are updated separately using the emitted
styles-manifest.json. Original stylesheets and previous bundles are left intact.
"""
...
FONT_CSS = ROOT / "font-manrope.css"
...
def build():
    ...
    manifest = {
        "font_css": FONT_CSS.name if fonts_present else None,
        "preload_latin": metadata.get("preload_latin"),
        "pages": {},
    }
    for page, originals in PAGE_SOURCES.items():
        sources = ([FONT_CSS.name] if fonts_present else []) + originals
        sections = ["/* Generated by scripts/build-styles.py; edit the source stylesheets. */\n"]
        for source in sources:
            ...
            if re.search(r"^\s*@(charset|import)\b", content, flags=re.MULTILINE):
                raise ValueError(f"Cannot safely concatenate @charset/@import in {source}")
            sections.append(f"\n/* Source: {source} */\n{content}\n")
        data = "".join(sections).encode("utf-8")
        digest = hashlib.sha256(data).hexdigest()[:12]
        name = f"styles-{Path(page).stem}-{digest}.css"
    ...
    (ROOT / "styles-manifest.json").write_text(
